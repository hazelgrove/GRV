...
\begin{alignRL}
  \children(\Gamma,c) &= \{ \e=(c',v) :~ c=c', \Gamma(\e)=+ \} \\
  \parents(\Gamma,v) &= \{ \e=(c,v') :~ v=v', \Gamma(\e)=+ \}
\end{alignRL}
...
\begin{mathpar}
  ...
  \Inferrule{
    ...
    \Gamma,c,a^{*} \xrightarrow{\Down} \Gamma,(v',i),a^{*}
  }
...
\begin{mathpar}
  \Inferrule{
    ...
    \Gamma,c,a^{*} \xrightarrow{\Create~k} \Gamma[\e :-> +],c,a^{*}
  }

  \Inferrule{
    \defaultIndex(c) = i' \\
    \children(\Gamma,c) = \{(c_j,v_j)\}_{j=1}^n \\
    \\\\
    \e = (c,(k,u)) \\
    c' = ((k,u),i') \\
    u \in \U \text{ fresh} \\
    ...
    \Gamma[\e :-> +][\e_j' :-> +]_{j=1}^n[\e_j :-> -]_{j=1}^n,c',a^{*}
  }
  ...
  \Inferrule{}{
    \Gamma,c,a^{*} \xrightarrow{\Restore~v} \Gamma[(c,v) :-> +],c,a^{*}
  }
\end{mathpar}
